...
class RBMAlgorithm(AlgoBase):
    ...
    def __init__(self, epochs=20, hiddenDim=100, learningRate=0.001, batchSize=100, sim_options={}):
        ...
        self.courseRecommendation.loadCourseData()
    ...
    def buildStoplist(self, trainset):
        ...
        for iid in trainset.all_items():
            ...
            courseID = trainset.to_raw_iid(iid)
            courseName = self.courseRecommendation.getCourseName(courseID)
            if courseName:
                courseName = courseName.lower()
                for term in self.stoplist:
                    if term in courseName:
                        print("Blocked ", courseName)
